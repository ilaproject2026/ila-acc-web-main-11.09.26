...
export const WorkStudyHub: React.FC = () => {
  // Hub Main Sub-Navigation
  const [hubTab, setHubTab] = useState<'packages' | 'hod' | 'promote_jd' | 'auto_trigger' | 'intake_tracking' | 'social_promo' | 'tie_ups'>('packages');
  ...
  const navItems: HubNavItem[] = [
    { id: 'packages', label: 'Package Management', icon: Layers, badge: packages.length },
    { id: 'hod', label: 'HOD Candidate Pipeline', icon: Briefcase, badge: candidates.length },
    { id: 'promote_jd', label: 'Promote JD (Marketing)', icon: Megaphone },
    { id: 'intake_tracking', label: 'Intake Desk', icon: Users },
    { id: 'auto_trigger', label: 'Auto Triggers', icon: Radio },
    { id: 'social_promo', label: 'Social Promo', icon: Send },
    { id: 'tie_ups', label: 'Corporate Tie-ups', icon: Building2 },
  ];
  ...
  return (
    <div className="flex flex-col w-full h-full min-h-[calc(100vh-12rem)] bg-slate-50 relative rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
      ...
      <HubSubNavBar
        items={navItems}
        activeTab={hubTab}
        onTabChange={(id:any) => setHubTab(id as any)}
      ...
      <div className="flex-1 w-full bg-slate-50 overflow-y-auto no-scrollbar relative p-4 md:p-6 space-y-6">
              ...
              <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto">
                {[
                  ...
                  <button
                    ...
                    className={`text-xs font-bold px-3.5 py-1.5 rounded-xl transition-all cursor-pointer whitespace-nowrap ${selectedCategoryFilter === cat.id
                    ...
                    {(['WhatsApp', 'Meta Ads', 'LinkedIn', 'Email Funnel', 'Public Contact Lists'] as JDPromotionPayload['channels'][number][]).map(ch => {
                      ...
                      return (
                        <button
                          ...
                          className={`p-3 rounded-xl text-xs font-bold border transition-all flex items-center justify-between cursor-pointer ${isSelected
        ...
        {hubTab === 'social_promo' && (
          <div className="animate-in fade-in">
            <HubSocialPromoView departmentName="Work While You Study" />
          </div>
        )}

        {/* SUB-TAB 7: CORPORATE & INSTITUTIONAL TIE-UPS */}
        {hubTab === 'tie_ups' && (
          <div className="animate-in fade-in">
            <HubHODView
              departmentName="Work and Study"
              departmentTagline="Work & Study Dual Degree Alliances, Corporate Stipend Sponsors & Apprenticeship Programs"
            />
          </div>
        )}
