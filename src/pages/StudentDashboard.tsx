...
import { Monitor, FileText, Bot, Mic, Send, Lock, Unlock, PlayCircle, Book, CheckCircle, Clock, BrainCircuit, Hand, Users, Video, LogIn, LogOut, GraduationCap, ShieldAlert, ArrowRight } from 'lucide-react'
...
export default function StudentDashboard() {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(() => {
    const role = localStorage.getItem('ilas_auth_role')
    return role === 'student' || role === 'team' || !!apiClient.getToken()
  })
  const [userRole, setUserRole] = useState<string | null>(() => localStorage.getItem('ilas_auth_role'))
  const [userName, setUserName] = useState<string>(() => localStorage.getItem('ilas_user_name') || 'Student Candidate')

  useEffect(() => {
    const checkAuth = () => {
      const role = localStorage.getItem('ilas_auth_role')
      const name = localStorage.getItem('ilas_user_name') || 'Student Candidate'
      const hasToken = !!apiClient.getToken()
      const isAuth = (role === 'student' || role === 'team') || hasToken
      setIsAuthenticated(isAuth)
      setUserRole(role)
      setUserName(name)
    }

    window.addEventListener('ilas-auth-state-changed', checkAuth)
    return () => window.removeEventListener('ilas-auth-state-changed', checkAuth)
  }, [])
  ...
  const openSignInModal = () => {
    window.dispatchEvent(new CustomEvent('open-portal-login', { detail: { tab: 'signin', role: 'student' } }))
  }

  const openRegisterModal = () => {
    window.dispatchEvent(new CustomEvent('open-portal-login', { detail: { tab: 'register', role: 'student' } }))
  }
  ...
  if (!isAuthenticated) {
    return (
      <div className="pt-24 min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-brand-950 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white/95 backdrop-blur-md rounded-3xl p-8 shadow-2xl border border-white/20 text-center animate-in zoom-in-95 duration-200">
          <div className="w-16 h-16 bg-brand-100 text-brand-700 rounded-2xl flex items-center justify-center mx-auto mb-5 shadow-inner">
          ...
          <h1 className="text-2xl font-black text-slate-900 mb-2">Access Student Classroom</h1>
          <p className="text-xs text-slate-600 mb-6 leading-relaxed">
            The Student Portal houses your live AI Coaching sessions, human tutor classrooms, active learning strategies, and syllabus materials. Please sign in or register as a student to continue.
          </p>

          <div className="space-y-3">
            <button
              onClick={openSignInModal}
              className="w-full py-3 bg-brand-700 hover:bg-brand-800 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-2 shadow-lg shadow-brand-700/25 transition-all cursor-pointer"
            >
              <LogIn className="w-4 h-4" /> Sign In to Student Portal
            </button>

            <button
              onClick={openRegisterModal}
              className="w-full py-3 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold rounded-xl text-xs flex items-center justify-center gap-2 transition-all cursor-pointer border border-slate-200"
            >
              <GraduationCap className="w-4 h-4 text-brand-700" /> Register New Student Account
            </button>
          </div>

          <div className="mt-6 pt-6 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400 font-medium">
  ...
  return (
    <div className="pt-20 bg-slate-50 min-h-screen pb-20">
      <div className="bg-brand-900 text-white py-8 mb-8 shadow-md">
        <div className="container-max px-4 flex flex-col md:flex-row justify-between items-center gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="px-2.5 py-0.5 rounded-full bg-brand-700/80 text-brand-200 text-[10px] font-extrabold uppercase tracking-wider border border-brand-500/30">
                {userRole === 'team' ? 'Staff / Admin View' : 'Student Candidate'}
              </span>
              <span className="text-xs text-brand-300 font-medium">Verified Active Session</span>
            </div>
            <h1 className="text-2xl md:text-3xl font-bold mb-1">Welcome back, {userName}!</h1>
            <p className="text-brand-100 text-xs md:text-sm">Access your live classes, AI tutor, and local library.</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <button 
              onClick={() => setClassMode('ai')} 
              className={`px-3.5 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-colors cursor-pointer ${classMode === 'ai' ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}
            >
              <BrainCircuit className="w-4 h-4" /> Intelli-Coach AI Mode
            </button>
            <button 
              onClick={() => setClassMode('tutor')} 
              className={`px-3.5 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-colors cursor-pointer ${classMode === 'tutor' ? 'bg-emerald-600 text-white shadow-md' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}
            >
            ...
            <button
              ...
              className="px-3 py-2 rounded-xl text-xs font-bold bg-red-500/20 hover:bg-red-500/30 text-red-200 border border-red-400/30 flex items-center gap-1.5 transition-colors cursor-pointer"
            >
